...
export abstract class Base<ElementType extends HTMLElement> {
    ...
    protected addInstance(): void {
        ...
        const moduleClass: string = 'h-' + this.getModuleName().toLowerCase();
        addClass([this.element], ['h-lib', moduleClass]);
        if (!isNullOrUndefined((<DomElements>(this.element as HTMLElement)).hel_instances)) {
            (<DomElements>(this.element as HTMLElement)).hel_instances.push(this);
        } else {
            setValue('hel_instances', [this], (<DomElements>(this.element as HTMLElement)));
        }
    }
    /**
    ...
    protected destroy(): void {
        // eslint-disable-next-line camelcase
        (<DomElements>(this.element as HTMLElement)).hel_instances =
            (<DomElements>(this.element as HTMLElement)).hel_instances ?
                (<DomElements>(this.element as HTMLElement)).hel_instances.filter((i: Object) => {
                    if (proxyToRaw) {
                ...
                : [];
        removeClass([this.element], ['h-' + this.getModuleName()]);
        if ((<DomElements>(this.element as HTMLElement)).hel_instances.length === 0) {
            // Remove module class from the root element
            removeClass([this.element], ['h-lib']);
        }
        this.clearChanges();
...
export function getComponent<T>(elem: HTMLElement | string, comp: string | any | T): T {
    ...
    if (ele && (<DomElements>(ele as HTMLElement)).hel_instances) {
        for (i = 0; i < (<DomElements>(ele as HTMLElement)).hel_instances.length; i++) {
            instance = <T>(ele as DomElements).hel_instances[parseInt(i.toString(), 10)];
...
export function removeChildInstance(element: HTMLElement): void {
    const childEle: any = [].slice.call(element.getElementsByClassName('h-control'));
    for (let i: number = 0; i < childEle.length; i++) {
        const compName: string = childEle[parseInt(i.toString(), 10)].classList[1].split('h-')[1];
