...
export interface Instance extends HTMLElement {
    ...
    hel_instances: { [key: string]: Object }[];
}
/**
...
export class Draggable extends Base<HTMLElement> implements INotifyPropertyChanged {
    ...
    protected bind(): void {
        this.toggleEvents();
        if (Browser.isIE) {
            addClass([this.element], 'h-block-touch');
        }
    ...
    private getScrollableValues(): void {
        ...
        const isModalDialog: boolean = this.element.classList.contains('h-dialog') && this.element.classList.contains('h-dlg-modal');
    ...
    private initialize(evt: MouseEvent & TouchEvent, curTarget?: EventTarget): void {
        ...
        if (evt.type !== 'touchstart' && this.isPreventSelect) {
            document.body.classList.add('h-prevent-select');
        }
        this.externalInitialize = false;
    ...
    private intDragStart(evt: MouseEvent & TouchEvent): void {
        ...
        if ((distance >= this.distance || this.externalInitialize)) {
            ...
            if (this.dragArea && typeof this.dragArea !== 'string' && this.dragArea.classList.contains('h-kanban-content') && this.dragArea.style.position === 'relative') {
    ...
    private bindDragEvents(dragTargetElement: HTMLElement): void {
        if (isVisible(dragTargetElement)) {
            ...
            this.toggleEvents();
            document.body.classList.remove('h-prevent-select');
        }
    }
    ...
    private intDrag(evt: MouseEvent & TouchEvent): void {
        ...
        if (this.helperElement.classList.contains('h-treeview')){
            if (this.dragArea) {
            ...
            if (this.dragArea) {
                const isDialogEle: boolean = this.helperElement.classList.contains('h-dialog');
                this.dragLimit.top = this.clone ? this.dragLimit.top : 0;
        ...
        if (this.dragArea && this.helperElement.classList.contains('h-treeview')) {
            const helperHeight: number = helperElement.offsetHeight + (parseFloat(styles.marginTop)
                    + parseFloat(styles.marginBottom));
        ...
        if (!this.elementInViewport(helperElement) && this.enableAutoScroll && !this.helperElement.classList.contains('h-treeview')) {
            this.helperElement.scrollIntoView();
        }

        let elements: NodeList | Element[] = document.querySelectorAll(':hover');
        if (this.enableAutoScroll && this.helperElement.classList.contains('h-treeview')) {
    ...
    private setDragArea(): void {
        ...
        if (ele) {
            ...
            eleHeightBound = ele.scrollHeight ? (this.dragArea && !isNullOrUndefined(this.helperElement) && this.helperElement.classList.contains('h-treeview')) ? ele.clientHeight : ele.scrollHeight : elementArea.bottom - elementArea.top;
            ...
            if (this.dragArea && !isNullOrUndefined(this.helperElement) && this.helperElement.classList.contains('h-treeview')) {
    ...
    private getHelperElement(evt: MouseEvent & TouchEvent): HTMLElement {
        ...
        if (this.clone) {
            if (this.helper) {
                ...
                element = createElement('div', { className: 'h-drag-helper h-block-touch', innerHTML: 'Draggable' });
    ...
    private checkTargetElement(evt: MouseEvent & TouchEvent): DropObject {
        ...
        if (!dropIns && target && !isNullOrUndefined(target.parentNode)) {
            const parent: Element = closest(target.parentNode, '.h-droppable') || target.parentElement;
            if (parent) {
    ...
    private getDropInstance(ele: Element): DropOption {
        ...
        let drop: Object;
        const eleInst: { [key: string]: Object }[] = ele && (<Instance>ele).hel_instances;
        if (eleInst) {
