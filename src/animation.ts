...
export class Animation extends Base<HTMLElement> implements INotifyPropertyChanged {
    ...
    public static stop(element: HTMLElement, model?: AnimationOptions): void {
        ...
        element.removeAttribute('h-animate');
        const animationId: string = element.getAttribute('h-animation-id');
        if (animationId) {
            const frameId: number = parseInt(animationId, 10);
            cancelAnimationFrame(frameId);
            element.removeAttribute('h-animation-id');
        }
        if (model && model.end) {
    ...
    private static applyAnimation(model: AnimationOptions): void {
        ...
        {
            model.element.setAttribute('h-animate', 'true');
        }
        const startAnimation: (timeStamp?: number) => void = (timeStamp?: number): void => {
            try {
                if (timeStamp) {
                    ...
                    if (model.timeStamp < duration && model.timeStamp + avg < duration && model.element.getAttribute('h-animate')) {
                        ...
                        model.element.removeAttribute('h-animation-id');
                        model.element.removeAttribute('h-animate');
                    ...
                    model.element.setAttribute('h-animation-id', timerId.toString());
                }
            } catch (e) {
                cancelAnimationFrame(timerId);
                if (model && model.element) {
...
function rippleHandler(e: MouseEvent): void {
    ...
    element.classList.add('h-ripple');
    ...
    const rippleElement: HTMLElement = createElement('div', { className: 'h-ripple-element', styles: styles });
...
function removeRipple(e: MouseEvent, eventArgs: RippleArgs): void {
    ...
    const element: HTMLElement = selector ? <HTMLElement>closest(target, selector) : target;
    if (!element || (element && element.className.indexOf('h-ripple') === -1)) {
        return;
    }
    const rippleElements: HTMLElement[] = selectAll('.h-ripple-element', element);
    ...
    setTimeout(() => {
        ...
        if (!element.getElementsByClassName('h-ripple-element').length) {
            element.classList.remove('h-ripple');
        }
        if (eventArgs.done) {
            eventArgs.done(e);
        }
...
/**
 * Animation Module provides support to enable ripple effect functionality to Helatra components.
 *
