...
export class Styler extends Component<HTMLElement> implements INotifyPropertyChanged {
    ...
    public render(): void {
        this.element.classList.add('h-styler');
        this.element.style.fontSize = this.size;
    }
    public destroy(): void {
        this.element.classList.remove('h-styler');
        ////toCover coverage
        this.clearTemplate();
        super.destroy();
    }
...
describe('Component', () => {
    ...
    it('instance creation check in appendTo', () => {
        ...
        expect(elem.hel_instances[0]).toEqual(styleObj);
        expect(elem.hel_instances.length).toEqual(1);
    });
    ...
    describe('refresh component', () => {
        ...
        it('before refresh', () => {
            expect(elem.classList.contains('h-lib')).toBe(true);
            expect(elem.classList.contains('h-styler')).toBe(true);
            expect(elem.classList.contains('h-control')).toBe(true);
        });
        it('after refresh', () => {
