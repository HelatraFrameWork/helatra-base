...
describe('draggable', () => {
    ...
    describe('Initialize of draggable', () => {
        ...
        it('class name', () => {
            expect(element.classList.contains('h-lib')).toBe(true);
            expect(element.classList.contains('h-draggable')).toBe(true);
        });
        it('check Event bind', () => {
        ...
        it('check element instance', () => {
            expect((<any>element).hel_instances[0].constructor.name).toBe('Draggable');
        });
        afterAll(() => {
            element.remove();
        });
    ...
    describe('check MouseMove event', () => {
        ...
        describe('Drag Start', () => {
            ...
            it('Drag start movement called properly based on the distance moved and without element cloning', () => {
                ...
                expect(dragstartEvent).toHaveBeenCalled();
                instance.intDestroy(mousedown);
                expect(document.body.classList.contains('h-prevent-select')).toBe(false);
            });
...
describe('droppable', () => {
    ...
    it('initialize droppable', () => {
        expect(dropElem.classList.contains('h-lib')).toBe(true);
        expect(dropElem.classList.contains('h-droppable')).toBe(true);
    });
    it('check element instance', () => {
        expect(dropElem.hel_instances[0].constructor.name).toBe('Droppable');
    });
    it('check drop event', () => {
    ...
    describe('intDrag and intDrop and ', () => {
        ...
        describe('check destroy method', () => {
            it('Destroy for Draggable', () => {
                ...
                expect(element.classList.contains('h-draggable')).toBe(false);
            });
            it('Destroy for Droppable', () => {
                let instance: Droppable = new Droppable(element);
                instance.destroy();
                expect(element.classList.contains('h-droppable')).toBe(false);
            });
        });
        afterEach(() => {
            document.body.innerHTML = '';
