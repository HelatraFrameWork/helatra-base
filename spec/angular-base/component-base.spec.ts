...
describe('=> Simple Component => ', () => {
    ...
    it('directive initialize', () => {
        expect(el.firstElementChild.classList.contains('h-control')).toBe(true);
        expect((el.firstElementChild as any).hel_instances[0].text).toBe(comp.text);
    });
    ...
    it('twoway binding from customer component', (done: Function) => {
        comp.text = 'Content';
        let sypFunction: Function = spyOn((el.firstElementChild as any).hel_instances[0], 'onPropertyChanged');
        fixture.detectChanges();
        setTimeout(
            () => {
    ...
    it('twoway binding setter not called when old and newvalue same', (done: Function) => {
        let sypFunction: Function = spyOn((el.firstElementChild as any).hel_instances[0], 'onPropertyChanged');
        (el.firstElementChild as any).hel_instances[0].text = 'EJButton';
        fixture.detectChanges();
        setTimeout(
            () => {
    ...
    it('twoway binding set undefined value', (done: Function) => {
        let sypFunction: Function = spyOn((el.firstElementChild as any).hel_instances[0], 'onPropertyChanged');
        (el.firstElementChild as any).hel_instances[0].text = undefined;
        fixture.detectChanges();
        setTimeout(
            () => {
...
describe('=> Simple Component => ', () => {
    ...
    beforeEach((done: Function) => {
        ...
        fixture.detectChanges();
        setTimeout(() => {
            destroySpy = spyOn((el.firstElementChild as any).hel_instances[0], 'destroy');
            done();
        }, 50);
    });
