...
describe('=> Form Component => ', () => {
        ...
        TestBed.overrideComponent(AppComponent, {
            set: {
                template: `<form #heroForm='ngForm' (ngSubmit)='onFormSubmit(heroForm.value)' >
                            ...
                            <hel-button [value]='text' name='button' [ngModel]='text'> </hel-button>
    ...
    it('onFocus and onBlur event call', () => {
        ...
        let instance = de.nativeElement.querySelector('.h-control').hel_instances[0];
...
describe('=> Form base => ', () => {
    ...
    it('twoway binding setter called when old and newvalue same', (done: Function) => {
        let sypFunction: Function = spyOn((el.firstElementChild.children[0] as any).hel_instances[0], 'onPropertyChanged');
        (el.firstElementChild.children[0] as any).hel_instances[0].text = 'Text';
        fixture.detectChanges();
        setTimeout(
            () => {
    ...
    it('twoway binding setter not called when old and newvalue same', (done: Function) => {
        let sypFunction: Function = spyOn((el.firstElementChild.children[0] as any).hel_instances[0], 'onPropertyChanged');
        (el.firstElementChild.children[0] as any).hel_instances[0].text = 'input';
        fixture.detectChanges();
        setTimeout(
            () => {
